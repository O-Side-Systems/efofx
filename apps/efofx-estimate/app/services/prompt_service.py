...
_REQUIRED_FIELDS = {"version", "name", "created_at", "system_prompt", "user_prompt_template"}
...
class PromptService:
    ...
    Call ``load_all()`` once at app startup. Use ``get()`` to retrieve prompts at runtime.
    """
    ...
    def load_all(cls, prompts_dir: str) -> None:
        ...
        Raises:
            ValueError: If a prompt file is missing required fields or if an already-loaded
        ...
        for json_file in path.glob("*.json"):
            ...
            try:
                ...
                raise ValueError(f"Invalid JSON in prompt file {json_file.name}: {e}") from e

            # Validate required fields
            missing = _REQUIRED_FIELDS - set(data.keys())
            if missing:
                raise ValueError(
                    f"Prompt file {json_file.name} is missing required fields: {missing}"
                )
        ...
        logger.info("Prompt registry loaded: %d prompt(s) from %s", loaded_count, prompts_dir)
    ...
    def get(cls, name: str, version: str = "latest") -> dict:
        ...
        Args:
            ...
            version: Exact semver string (e.g., "1.0.0") or "latest" (resolves to highest).

        Returns:
            The prompt dict (contains version, name, system_prompt, user_prompt_template, etc.)
        ...
        if version == "latest":
            ...
            if not candidates:
                raise KeyError(f"Prompt not found: {name}:latest (no versions registered)")
