...
CALB-02: Returns mean_variance_pct, accuracy_buckets, by_reference_class breakdown,
         and get_trend() monthly time-series with period, mean_variance_pct, outcome_count.  # noqa: E501
...
def _compute_accuracy_buckets(variances: list[float]) -> dict:
    """Compute exclusive accuracy bucket proportions from a list of variance percentages.  # noqa: E501
...
class CalibrationService:
    ...
    def _build_pipeline(self, date_filter: dict, tenant_id: str) -> list[dict]:
        ...
        # Stage 3: Unwind — preserveNullAndEmptyArrays keeps feedback without matching RC  # noqa: E501
        pipeline.append(
            {
                "$unwind": {
    ...
    def _build_trend_pipeline(self, since: datetime) -> list[dict]:
        ...
        pipeline.append({"$match": {"submitted_at": {"$gte": since}}})
    ...
    async def get_metrics(
        ...
        overall_mean = (
            round(sum(all_variances) / len(all_variances), 1) if all_variances else 0.0
        )
