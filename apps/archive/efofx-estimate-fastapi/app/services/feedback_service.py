...
class FeedbackService:
    ...
    async def get_feedback_summary(self, tenant: Tenant) -> FeedbackSummary:
        ...
        try:
            ...
            average_rating = (
                sum(f.get("rating", 0) for f in feedback_list) / total_feedback
            )

            # Calculate accuracy averages
            cost_accuracies = [
                f.get("cost_accuracy") for f in feedback_list if f.get("cost_accuracy")
            ]
            timeline_accuracies = [
                f.get("timeline_accuracy")
                for f in feedback_list
                if f.get("timeline_accuracy")
            ]
            ref_class_accuracies = [
                f.get("reference_class_accuracy")
                for f in feedback_list
                if f.get("reference_class_accuracy")
            ]

            cost_accuracy_avg = (
                sum(cost_accuracies) / len(cost_accuracies) if cost_accuracies else None
            )
            timeline_accuracy_avg = (
                sum(timeline_accuracies) / len(timeline_accuracies)
                if timeline_accuracies
                else None
            )
            reference_class_accuracy_avg = (
                sum(ref_class_accuracies) / len(ref_class_accuracies)
                if ref_class_accuracies
                else None
            )
            ...
            for f in feedback_list:
                ...
                feedback_by_type[feedback_type] = (
                    feedback_by_type.get(feedback_type, 0) + 1
                )
    ...
    async def get_feedback_by_type(
        self, tenant: Tenant, feedback_type: str
    ) -> List[Feedback]:
    ...
    async def get_feedback_analytics(
        self, tenant: Tenant, days: int = 30
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            accuracy_trends = {
                "cost_accuracy": self._calculate_accuracy_trend(
                    feedback_list, "cost_accuracy"
                ),
                "timeline_accuracy": self._calculate_accuracy_trend(
                    feedback_list, "timeline_accuracy"
                ),
                "reference_class_accuracy": self._calculate_accuracy_trend(
                    feedback_list, "reference_class_accuracy"
                ),
            }
            ...
            for f in feedback_list:
                feedback_type = f.get("feedback_type", "unknown")
                feedback_distribution[feedback_type] = (
                    feedback_distribution.get(feedback_type, 0) + 1
                )
    ...
    async def store_feedback_with_snapshot(
        ...
        doc = FeedbackDocument(
            ...
            discrepancy_reason_secondary=(
                submission.discrepancy_reason_secondary.value
                if submission.discrepancy_reason_secondary
                else None
            ),
    ...
    def _calculate_accuracy_trend(
        self, feedback_list: List[Dict], accuracy_field: str
    ) -> Dict[str, float]:
