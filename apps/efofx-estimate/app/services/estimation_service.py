...
class EstimationService:
    ...
    async def get_estimation(
        ...
        try:
            ...
            return EstimationResponse(
                ...
                message=API_MESSAGES.get(
                    f"ESTIMATION_{session.status.upper()}",
                    "Estimation status retrieved",
                ),
                result=session.result,
                next_action=(
                    None
                    if session.status == EstimationStatus.COMPLETED
    ...
    async def upload_image(
        self, session_id: str, file: UploadFile, tenant: Tenant
    ) -> str:
        ...
        try:
            ...
            await collection.update_one(
                {"session_id": session_id}, {"$push": {"images": image_url}}
            )
    ...
    async def generate_from_chat(
        ...
        estimation_output = await self.llm_service.generate_estimation(
            ...
            region=region,
            reference_data=(
                {"reference_projects": reference_projects[:5]}
