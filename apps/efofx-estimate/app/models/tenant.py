...
class Tenant(BaseModel):
    ...
    api_key_last6: str = Field(
        default="", description="Last 6 chars of raw API key for display masking"
    )
    tier: str = Field(
        default="trial", description="Subscription tier: 'trial' or 'paid'"
    )
    ...
    openai_key_last6: Optional[str] = Field(
        default=None,
        description="Last 6 chars of raw OpenAI key for masked display (sk-...abc123)",
    )
    is_active: bool = Field(
        default=True, description="Whether tenant account is active"
    )
