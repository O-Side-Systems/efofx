...
class LLMService:
    ...
    def __init__(self, api_key: str, tenant_id: str) -> None:
        """api_key and tenant_id are REQUIRED — no fallback to settings.OPENAI_API_KEY."""  # noqa: E501
    ...
    async def classify_project(
        self, description: str, region: str, reference_classes: list
    ) -> str:
        """Classify project into reference class using LLM."""
        try:
            prompt = f"""Analyze the following project description and classify it into the most appropriate reference class.  # noqa: E501
...
Please provide only the reference class name as your response."""
            ...
            system_message = (
                "You are an expert construction estimator. Your task is to classify construction "  # noqa: E501
                "projects into appropriate reference classes based on project descriptions and "  # noqa: E501
                "regional context. Respond with only the reference class name, nothing else."  # noqa: E501
            )
    ...
    async def generate_estimation(
        ...
        system_prompt = (
            ...
            "reference class, and region, produce a detailed cost estimate with P50/P80 ranges. "  # noqa: E501
            "Break costs down by category. Include adjustment factors as named multipliers. "  # noqa: E501
            "List all assumptions explicitly. Assign a confidence score (0-100) based on "  # noqa: E501
            "how much information you have to work with. Omit inapplicable cost categories "  # noqa: E501
...
Region: {region}"""
        ...
        if use_cache:
            ...
            if cached is not None:
                logger.debug(
                    "Cache hit for estimation request (key=%s...)", input_hash[:8]
                )
                return EstimationOutput.model_validate_json(cached)

        try:
            completion = await self.client.beta.chat.completions.parse(
                model=settings.OPENAI_MODEL,  # gpt-4o-mini (required for structured outputs)  # noqa: E501
                messages=messages,
            ...
            if result is None:
                ...
                raise ValueError(
                    f"LLM refused or failed to parse structured output: {refusal}"
                )

            # Store in cache
            if use_cache:
                await valkey_cache.set(
                    self.tenant_id, input_hash, result.model_dump_json()
                )
