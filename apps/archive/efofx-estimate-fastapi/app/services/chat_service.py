...
class ChatService:
    ...
    async def send_message(self, request: ChatRequest, tenant: Tenant) -> ChatResponse:
        ...
        try:
            ...
            if "Auth" in error_type_str or "quota" in str(e).lower():
                ...
                response_content = "We're having trouble generating a response. Please try again in a moment."  # noqa: E501
            # Conversation is ALWAYS preserved — remove the last user message wasn't added yet  # noqa: E501
            session.messages.append(
                ChatMessage(role="assistant", content=response_content)
            )
            await self._persist_session(session, tenant)
        ...
        if session.scoping_context.is_ready():
            ...
            if not was_ready:
                ...
                llm_response = (
                    llm_response.rstrip()
                    + "\n\nI have enough details to generate an estimate for your project. Shall I go ahead?"  # noqa: E501
                )
                session.status = "ready"
    ...
    async def get_chat_history(
        ...
        collection = get_tenant_collection(
            DB_COLLECTIONS["CHAT_SESSIONS"], tenant.tenant_id
        )
    ...
    def _update_scoping_context(
        ...
        Only updates fields that are currently None — does not overwrite existing context.  # noqa: E501
        """
        ...
        if ctx.project_size is None:
            # Match patterns like "15x30", "15 x 30", "500 sq ft", "500 sqft", "500 square feet"  # noqa: E501
            size_patterns = [
    ...
    def _is_explicit_estimate_trigger(self, message: str) -> bool:
        ...
        return any(
            trigger in message_lower for trigger in self.ESTIMATE_TRIGGER_PHRASES
        )
    ...
    def _is_confirmation(self, message: str) -> bool:
        ...
        for word in self.CONFIRMATION_WORDS:
            if (
                message_lower == word
                or message_lower.startswith(word + " ")
