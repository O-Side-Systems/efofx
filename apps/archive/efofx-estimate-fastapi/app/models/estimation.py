...
class CostCategoryEstimate(BaseModel):
    ...
    percentage_of_total: float = Field(
        description="This category's share of total cost (0.0-1.0)"
    )
...
class AdjustmentFactor(BaseModel):
    ...
    name: str = Field(
        description="Adjustment name (e.g., 'Urban premium', 'Complexity multiplier')"
    )
    multiplier: float = Field(
        description="Multiplier value (e.g., 1.15 for 15% increase)"
    )
...
class EstimationOutput(BaseModel):
    ...
    confidence_score: float = Field(
        ge=0,
        le=100,
        description="Confidence score 0-100 reflecting information completeness",
    )
    assumptions: List[str] = Field(
        description="Explicit assumptions the estimate is based on"
    )
    summary: str = Field(
        description="One-paragraph plain-language summary of the estimate"
    )
...
class EstimationSession(BaseModel):
    ...
    status: EstimationStatus = Field(
        default=EstimationStatus.INITIATED, description="Session status"
    )
    ...
    prompt_version: Optional[str] = Field(
        None, description="Prompt version used for this estimate (e.g., '1.0.0')"
    )
    result: Optional[Any] = Field(
        None, description="Estimation result (legacy field, always None in new flow)"
    )
    chat_messages: List[str] = Field(
        default_factory=list, description="Chat message IDs"
    )
    ...
    class Config:
        ...
        schema_extra = {
            "example": {
                ...
                "status": "completed",
                "description": "I want to install a 15x30 foot pool with spa in my backyard.",  # noqa: E501
...
class EstimationResponse(BaseModel):
    ...
    result: Optional[Any] = Field(
        None, description="Estimation result if completed (legacy field)"
    )
