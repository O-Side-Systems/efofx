...
async def _validate_api_key(raw_key: str) -> Tenant:
    ...
    if len(parts) >= 3:
        ...
        try:
            ...
            if tenant_doc and _password_hash.verify(
                raw_key, tenant_doc["hashed_api_key"]
            ):
                if not tenant_doc.get("email_verified", False):
                    raise HTTPException(
                        status_code=403,
                        detail="Email not verified. Please verify your email before using the API.",  # noqa: E501
                    )
...
async def get_current_tenant_optional(
    ...
    Use on endpoints that have different behavior for authenticated vs anonymous callers.  # noqa: E501
    """
    if not credentials:
        return None
